...
fn test_jump_assemble() {
    let test_vec = vec![("JMI $128", 0x5080, None), 
                        ("JEQ $4096", 0, Some(SemanticError::IMMEDIATEOVERSIZE((0, 0)))),
    ...
    test_vec.iter().for_each(|(instr, res, err)| test_assemble(instr, *res, err));
}
...
fn test_assemble(instruction: &str, encoding: u16, err: &Option<SemanticError>) {
    ...
    let mut prog = parse::parse::program()
        .easy_parse(position::Stream::new(instruction)).unwrap().0;
