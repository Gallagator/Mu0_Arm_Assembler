...
use combine::{ParseError, Parser, Stream, attempt, choice, from_str, many, many1, optional, parser::char::{alpha_num, digit, hex_digit, letter}, token, tokens}; use std::str::Chars;
...
fn int_lit<Input>() -> impl Parser<Input, Output = i16>
    where Input: Stream<Token = char>,
          ...
          ).map(|(neg, _, num): (Option<_>, _, i16)| 
                if let None = neg { num } else { -num } 
                )
          //( 
          //    tag("0x"),
